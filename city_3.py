...
fieldnames=('pk','觀測時間(day)','測站氣壓(hPa)','海平面氣壓(hPa)','測站最高氣壓(hPa)','測站最高氣壓時間(LST)','測站最低氣壓(hPa)','測站最低氣壓時間(LST)','氣溫(℃)','最高氣溫(℃)','最高氣溫時間(LST)','最低氣溫(℃)','最低氣溫時間(LST)'\
            ...
            '''
fieldnames=('ig','TOWN_CODE','LOCATION','ABE_DA','AEN_DA','AREA_TA','UN_NA','LASTMOD','POLY_LIST')
reader=csv.DictReader(record_title, fieldnames)
for i in reader:
    print(type(i))
    data={
        'json':json.dumps(i)
    ...
    print(data)
    response=requests.post('http://localhost:8080/test2.test', data=data)
